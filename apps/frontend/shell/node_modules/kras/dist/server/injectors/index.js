...
function normalizeRequest(targets, req) {
    ...
    const headers = deepMerge({ ...req.headers }, req.addedHeaders);
    const query = deepMerge({ ...req.query }, req.addedQuery);
...
async function tryInjectors(injectors, req) {
...
async function addInjectorInstance(Injector, options, config, server) {
    if (Injector) {
...
export async function withInjectors(server, config) {
